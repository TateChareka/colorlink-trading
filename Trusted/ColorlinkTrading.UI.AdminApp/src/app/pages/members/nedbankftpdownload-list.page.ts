...
@Component({
  selector: 'nedbankftpdownload-list',
  templateUrl: './nedbankftpdownload-list.page.html',
  styleUrls: ['./../../../theme/datagrid-toolbar.css']
})

export class NedbankFTPDownloadListPage {
  ...
  constructor(private routeControl: Router, private apiProvider: APIMembersProvider, private apiUtility: APIUtilityProvider) {
    appGlobals.SetActivePageTitle("Nedbank FTP Downloads");
      ...
      load: function (loadOptions) {
        ...
        return that.apiUtility.UtilityProvider_GetPeriodDates(that.SearchPeriod).then(
          (result: any) => {
            ...
            //filter the data
            return that.apiProvider.NedbankFTPDownload_Search(pageSize, pageNumber, orderField, orderDirection,
              ...
              (result: any) => {
                ...
                return {
                  data: result.NedbankDownloadForFTPTransmission,
  ...
  exportToExcel() {
    ...
    this.apiProvider.NedbankFTPDownload_Export(this.OrderField, this.OrderDirection, this.SearchText, this.SearchDateFrom, this.SearchDateTo).then(
      (result: any) => {
