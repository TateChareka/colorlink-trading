...
@Component({
  selector: 'memberstatements-list',
  templateUrl: './memberstatements-list.page.html',
  styleUrls: ['./../../../theme/datagrid-toolbar.css']
})

export class MemberStatementsListPage {
  ...
  ActionRow: any = {};
  OrderField: string = "StatementEndDate";
  ...
  constructor(private routeControl: Router, private apiProvider: APIMembersProvider, private apiUtility: APIUtilityProvider) {
    appGlobals.SetActivePageTitle("Member Statements");
  ...
  ngAfterContentInit() {
    ...
    this.dataStore = new DataSource({
      load: function (loadOptions) {
        var orderField = loadOptions.sort ? loadOptions.sort[0].selector : "StatementEndDate";
        ...
        return that.apiUtility.UtilityProvider_GetPeriodDates(that.SearchPeriod).then(
          (result: any) => {
            ...
            //filter the data
            return that.apiProvider.MemberStatements_Search(pageSize, pageNumber, orderField, orderDirection,
  ...
  exportToExcel() {
    ...
    this.apiProvider.MemberStatements_Export(this.OrderField, this.OrderDirection, this.SearchText, this.SearchDateFrom, this.SearchDateTo).then(
      (result: any) => {
  ...
  ViewStatement() {
    var res = (this.ActionRow.StatementFileName).replace(/\\/g, "/");
    var opened = window.open(("https://cdn.schooldays.co.za/CustomerStatements/" + res), "_blank");
  }
}