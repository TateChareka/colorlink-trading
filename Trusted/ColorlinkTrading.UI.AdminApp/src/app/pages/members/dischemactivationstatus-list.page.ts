...
@Component({
  selector: 'dischemactivationstatus-list',
  templateUrl: './dischemactivationstatus-list.page.html',
  styleUrls: ['./../../../theme/datagrid-toolbar.css']
})

export class DischemActivationStatusListPage {
  ...
  constructor(private routeControl: Router, private apiProvider: APIMembersProvider, private apiUtility: APIUtilityProvider) {
    appGlobals.SetActivePageTitle("Dis-Chem Activation Status");
      ...
      load: function (loadOptions) {
        ...
        return that.apiUtility.UtilityProvider_GetPeriodDates(that.SearchPeriod).then(
          (result: any) => {
            ...
            //filter the data
            return that.apiProvider.DischemActivationStatus_Search(pageSize, pageNumber, orderField, orderDirection,
              ...
              (result: any) => {
                ...
                return {
                  data: result.DischemActivationStatus,
  ...
  exportToExcel() {
    ...
    this.apiProvider.DischemActivationStatus_Export(this.OrderField, this.OrderDirection, this.SearchText, this.SearchDateFrom, this.SearchDateTo).then(
      (result: any) => {
