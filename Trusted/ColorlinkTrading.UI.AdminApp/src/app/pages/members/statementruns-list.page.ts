...
@Component({
  selector: 'statementruns-list',
  templateUrl: './statementruns-list.page.html',
  styleUrls: ['./../../../theme/datagrid-toolbar.css']
})

export class StatementRunsListPage {
  ...
  OrderField: string = "StatementRunId";
  ...
  constructor(private routeControl: Router, private apiProvider: APIMembersProvider, private apiUtility: APIUtilityProvider) {
    appGlobals.SetActivePageTitle("Statement Runs");
     ...
     ngAfterContentInit() {
        ...
        this.dataStore = new DataSource({
          load: function (loadOptions) {
            var orderField = loadOptions.sort ? loadOptions.sort[0].selector : "StatementRunId";
            ...
            return that.apiUtility.UtilityProvider_GetPeriodDates(that.SearchPeriod).then(
              (result: any) => {
                ...
                //filter the data
                return that.apiProvider.StatementRuns_Search(pageSize, pageNumber, orderField, orderDirection,
      ...
      exportToExcel() {
       appGlobals.ShowGlobalLoader();
        this.apiProvider.StatementRuns_Export(this.OrderField, this.OrderDirection, this.SearchText, this.SearchDateFrom, this.SearchDateTo).then(
          (result: any) => {
